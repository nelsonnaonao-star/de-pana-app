  ...
  if (partnerIds.length > 0) {
    ...
    deduped = deduped.map(chat => {
      ...
      return {
        ...
        name: finalName,
        ...(profile
          ? { avatar: resolvedAvatar, is_online: profile.status === "online" }
          : { avatar: "" }),
      };
    });
  }
...
export async function createChat(chat: Partial<Chat>): Promise<Chat> {
  ...
  if (chat.profile_id && chat.admin_id && chat.profile_id !== chat.admin_id) {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      // If the current user has this chat hidden (deleted for them), unhide it
      // but preserve cleared_at as the permanent history boundary
      const userId = chat.admin_id;
    ...
    if (partRows && partRows.length >= 2) {
      ...
      if (duplicates.length > 0) {
        const { data: existingViaParticipants } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        if (existingViaParticipants) {
          // Unhide if the current user had this chat hidden
          const userId = chat.admin_id;
          const { data: clearRow2 } = await supabase
            .from("chat_clears")
            .select("hidden")
            .eq("chat_id", existingViaParticipants.id)
            .eq("user_id", userId)
            .maybeSingle();
          if (clearRow2?.hidden) {
            await supabase
          ...
          return existingViaParticipants as Chat;
        }
      }
    }
  }
