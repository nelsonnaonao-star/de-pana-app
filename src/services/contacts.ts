    ...
    if (profiles && profiles.length > 0) {
      ...
      return contacts.map(c => {
        ...
        return {
          ...
          // El nombre personalizado guardado por el usuario tiene prioridad
          // absoluta sobre profiles.name. Solo se usa profiles.name como
          // fallback si el usuario nunca guardó un nombre para este contacto.
          name: c.name || (profile as any)?.name || "",
        };
      });
    }
  }
