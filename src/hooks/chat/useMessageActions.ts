...
export function useMessageActions(params: UseMessageActionsParams): UseMessageActionsReturn {
  ...
  const handleSendLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        await messageRepo.upsertMessage(chatId, { ...newMsg, clientId, sender_id: uid }, uid);
  ...
  const handleSendText = async (textOverride?: string) => {
    ...
    try {
      ...
      // Encolar en SyncService para reintento automático con persistencia
      try {
        await syncService.queueMessage(chatId, { ...newMsg, clientId, sender_id: uid }, uid);
      } catch (qErr) {
        console.error("[CHAT] SyncService queue failed:", qErr);
      }
    } finally {
      pendingSendIdsRef.current.delete(tempId);
      inFlightMessageIds.delete(tempId);
     ...
     if (type === "image") {
       ...
       if (isCapacitor) {
        ...
        try {
          ...
          messageRepo.upsertMessage(chatId, sendingMsg, uid);
pendingSendIdsRef.current.add(tempId);
    inFlightMessageIds.add(tempId);

          const url = await uploadChatMedia(fileBlob, "image");
          const mediaUpdated = { ...sendingMsg, mediaUrl: url };
          await messageRepo.upsertMessage(chatId, { ...mediaUpdated, clientId: clientIdCamera, sender_id: uid }, uid);
          setMessages(prev => {
            if (!prev.some(m => m.id === tempId)) return prev;
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        try {
          ...
          await messageRepo.upsertMessage(chatId, { ...mediaUpdated, clientId: clientIdFile, sender_id: uid }, uid);
          setMessages(prev => {
            if (!prev.some(m => m.id === tempId)) return prev;
            return prev.map(m => m.id === tempId ? mediaUpdated : m);
          });
  ...
  const handleFinishVoiceNote = async () => {
    ...
    pendingSendIdsRef.current.add(tempId);
    inFlightMessageIds.add(tempId);
    await messageRepo.upsertMessage(chatId, newMsg, uid);
    // Persistir blob en IDB para reintentos offline (ANTES del upload)
    await messageRepo.saveAudioBlob(tempId, blob);
